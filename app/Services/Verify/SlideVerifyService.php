...
/** 
 * - 示例
 * \App\Services\Verify\SlideVerifyService::instance('init');
 *      String 获取验证码的html
 *
 * \App\Services\Verify\SlideVerifyService::instance('check');
 *      Array  验证验证码结果   => {["Err"=>"状态码","out"=>"对应的错误信息"]}
 *
 * \App\Services\Verify\SlideVerifyService::instance('is_pass');
 *      Array  查询验证结果  => ["status"=> true|false,,"Err"=>"状态码","out"=>"对应的错误信息"]
 */

use App\Services\Verify\SupportVerfiyService;
...
class SlideVerifyService
{
    ...
    public static $times_max  = 3; // 单个验证图片，最多可验证次数
    public static $timer      = 60; // 验证码的时效，单位s
    /**
     * 自动删除临时资源
     */
    ...
    public static function instance($option = 1, $more = 8)
    {
        ...
        if ('' != SupportVerfiyService::get_instance()->get_data(self::$session_data)) {
            // $this_session = json_decode($_SESSION[self::$session_data], true);
            $_data = SupportVerfiyService::get_instance()->get_data(self::$session_data) ?? '';
            $this_session = json_decode($_data, true);
        ...
        switch ($option) {
            ...
            case 'init':
                ...
                SupportVerfiyService::get_instance()->set_data([
                    self::$session_data => $init,
                ]);
                $obj = new self;
                return $obj->render();
            case 2:
    ...
    protected function restruct_cut()
    {
        ...
        SupportVerfiyService::get_instance()->set_data([
            self::$session_name => $this->location_x,
        ]);
    ...
    private static function check($val, $deviation)
    {
        if (!is_numeric($val) || !is_numeric($deviation)) {
            ...
            $_data = SupportVerfiyService::get_instance()->get_data(self::$session_name);
            $max   = intval($_data) + $deviation;
            $min   = intval($_data) - $deviation;
            // 是否在误差允许范围内
            if('' == $_data){
                $msg['Err'] = 2001;
                $msg['out'] = self::get_lan_pack()['code']['c_' . $msg['Err']];
            }
            elseif ($val <= $max && $val >= $min) {
                $msg['Err'] = 2000;
                $msg['out'] = self::get_lan_pack()['code']['c_' . $msg['Err']];
            } else {
                $msg['Err'] = 2001;
    ...
    private static function validate($session, $x_value, $fault_tolerant)
    {
        ...
        if ($result['Err'] == 2000) {
            ...
            SupportVerfiyService::get_instance()->set_data([
                self::$session_data => json_encode($session),
            ]);
    ...
    private static function if_success($session)
    {
        ...
        if ($session['success']) {
            ...
            // $_SESSION[self::$session_data] = json_encode($session);
            SupportVerfiyService::get_instance()->set_data([
                self::$session_name => mt_rand(1, 100), // 设置临时验证码，防止验证码重复提交
                self::$session_data => json_encode($session),
            ]);
